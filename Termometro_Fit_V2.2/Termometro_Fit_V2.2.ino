...
byte tempoholdaceso = 5; // tempo que o circuito mantem o farol baixo aceso mesmo com iluminacao no LDR
...
void loop(){
  ...
  // Acende o Led Verde conforme opcao de auto ou off
  if (farolauto == 1) {
    analogWrite(ledr2, 255);
  }
  if (farolauto == 0) {
  ...
  //  analogWrite(ledr2, 0); // apaga o Led vermelho do IR
  ...
  digitalWrite(ledr2, LOW); // apaga o Led vermelho do IR
...
void sensorluz() { // Rotina do sensor de luz
  ...
  if (ldrvalor < 800) {
    segundo3 = segundo + tempoholdaceso;
    pwm = 210;
    ledpwm = 1;
  ...
  if (ldrvalor > 850 && segundo > segundo3) {
    pwm = 0;
    ledpwm = 4;
