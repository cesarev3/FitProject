...
const int ledr = 10;
const int ledg = 6;
const int lanterna = 2;
const int brilho = 9;
const int LDR = A3;
const int farol = 11;
const int LM351 = A1;
const int LM352 = A2;
const int lanternain = A6;
const int farolin = A7;
...
byte pwm = 255;
byte ledpwm = 4;
byte senscontauto = 1; // contador do modo auto
byte senscont = 3; // contador sensibilidade farol baixo
byte senscontlant = 9; // contador sensibilidade Lanterna
int sensvalor = 200; // sensibilidade do LDR para farol baixo
int sensvalorlant = 650; // sensibilidade do LDR para lanterna
int desligalant = 700; // variavel com valor para desligar a lanterna
int desligafarol = 400; // variavel com valor para desligar o farol baixo
boolean sensmenu = 0;
byte menu = 0;
boolean hideon = 0;
...
boolean farolauto = 1;
boolean ledfarol = 0;
boolean farolbaixoon = 0;
boolean lanternaon = 0;
boolean farolaltolog = 0;
boolean lanternalog = 0;
int lanternavalor = 0;
int farolaltovalor = 0;


// Tempos em segundos
byte tempotela1 = 10;
byte tempotela2 = 20; //este tempo deve ser somado ao tempotela1. Para 10 segundos deve ser colocado tempotela1 + 10
byte delayaceso = 20; // tempo que o circuito mantem o farol baixo aceso mesmo com iluminacao no LDR
...
void loop(){
  ...
  // verifica entradas dos acopladores
  lanternavalor = analogRead(lanternain); 
  farolaltovalor = analogRead(farolin);

  if (lanternavalor < 200) {
    lanternalog = 0;
  }
  else {
    if (lanternavalor > 500) {
      lanternalog = 1;
    }
  }

  if (farolaltovalor > 500) {
    farolaltolog = 0;
  }
  else {
    if (farolaltovalor < 200) {
      farolaltolog = 1;
    }
  } 
  ...
  if (irrecv.decode(&results)) { 
    ...
    if (menu == 1) {  // le os dados se estiver na rotina sensibilidade do sensor para farol baixo
    ...
    if (menu == 2) {  // le os dados se estiver na rotina sensibilidade do sensor para lanterna
    ...
    if (menu == 3) {  // le os dados se estiver na rotina delay do farol
    ...
    if (menu == 4) {  // le os dados para ativar o modo auto dos farois
  ...
  //Serial.print(" Farol Sens: ");
  //Serial.print(sensvalor);
  //Serial.print(" Lanterna Sens: ");
  //Serial.print(sensvalorlant);
  ...
  //Serial.print(lanternavalor);
  //Serial.print(" farolalto: ");
  //Serial.print(farolaltovalor);
  //Serial.print(" Opto Lanterna: ");
  //Serial.print(lanternalog);
  //Serial.print(" Opto Farol: ");
  //Serial.print(farolaltolog);
  //Serial.print(" Lanterna ON: ");
  //Serial.print(lanternaon);
  //Serial.print(" farolbaixo ON: ");
  //Serial.print(farolbaixoon);
  //Serial.print(" ACC: ");
    ...
    if (menu == 1) {
      ...
      if (menu == 2) {
        ...
        if (menu == 3) {
          ...
          if (menu == 4 && senscontauto == 1) {
            unidade = 1;
            dezena = 1;
          }
          else {
            if (menu == 4 && senscontauto == 0) {
              unidade = 0;
              dezena = 0;
            }  
          }
...
void sensorluz() { // Rotina do sensor de luz
  ...
  if (ldrvalor < sensvalorlant) {
    segundo4 = segundo + delayaceso;
    pwm = 210;
    ledpwm = 1;
  ...
  if (ldrvalor > desligalant && segundo > segundo4) {
    pwm = 0;
    ledpwm = 4;
  ...
  if (ldrvalor > desligafarol && segundo > segundo3) {
    pwm = 0;
    ledpwm = 4;
    analogWrite(brilho, pwm); 
    digitalWrite(farol, LOW);
